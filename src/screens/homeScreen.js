    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.searchContainer}>
                ...
                <TextInput style={styles.input} placeholder="Search country" placeholderTextColor="gray" value={input} 
                onChangeText={input => {
                    setInput(input);
                    handleSearch(input)
                }} />
            <SelectDropdown data={regions} 
            onSelect={(selectedItem) => {handleSelection(selectedItem); setSelected(selectedItem)}} 
            buttonTextAfterSelection={(item) => (item)} rowTextForSelection={(item) => (item)} 
            dropdownStyle={styles.dropDrown} buttonStyle={styles.buttonDropdown} buttonTextStyle={styles.buttonTextStyle}
            defaultButtonText='Select by continent' renderDropdownIcon={() => <SimpleLineIcons name='arrow-down' style={{left: Dimensions.get('window').height * 0.009}}/>}/>
            </View>

            <View style={styles.countriesListContainer}>
                <FlatList data={input == "" && selected == "All" ? countries : filteredCountries} renderItem={({ item }) => 
                <RenderCountriesList country={item} navigation={navigation} />} numColumns={2} keyExtractor={item => item.name.official}/>
            </View>
        </SafeAreaView>
    )
}
